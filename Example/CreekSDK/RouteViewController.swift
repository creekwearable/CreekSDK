...
class RouteUploadViewController: UIViewController, UIDocumentPickerDelegate {
   ...
   @objc private func selectFileTapped() {
       if #available(iOS 14.0, *) {
           if let gpxType = UTType(filenameExtension: "gpx") {
               let picker = UIDocumentPickerViewController(forOpeningContentTypes: [gpxType], asCopy: true)
               picker.delegate = self
               picker.modalPresentationStyle = .formSheet
               present(picker, animated: true, completion: nil)
           }
       } else {
           let picker = UIDocumentPickerViewController(documentTypes: ["public.xml"], in: .import)
           picker.delegate = self
           picker.modalPresentationStyle = .formSheet
           present(picker, animated: true, completion: nil)
       }
    ...
    @objc private func startUpload() {
       ...
       if let fileData = try? Data(contentsOf: fileURL){
          ...
          CreekInterFace.instance.getGPXEncodeUint8List(data: fileData, geoId: 8556380159,sportType: .IRUN,distance: 15000) { lat, lon in
             return "shengzheng"
          } encode: { model in
             self.view.hideRemark()
             CreekInterFace.instance.upLoadGeo(data: model, geoId: 8556380159) { progress in

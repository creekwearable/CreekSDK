...
class OTAUploadViewController: UIViewController, UIDocumentPickerDelegate {
   ...
   @objc private func selectFileTapped() {
       if #available(iOS 14.0, *) {
           if let gpxType = UTType(filenameExtension: "ota") {
    ...
    @objc private func startUpload() {
       ...
       if let fileData = try? Data(contentsOf: fileURL){
          CreekInterFace.instance.upload(fileName: "ota.ota", fileData: fileData) { progress in
